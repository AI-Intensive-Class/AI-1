...
# Modify this function to save individual images instead of creating a video
def save_images(frames, save_path, frame_idx, prefix="frame"):
    for i, frame in enumerate(frames):
        image_file = os.path.join(save_path, f"{prefix}_{frame_idx}_{i}.png")
        frame_image = Image.fromarray(frame.permute(1, 2, 0).cpu().numpy())
        frame_image.save(image_file)
...
for idx, image_file in enumerate(input_files):
    ...
    # Reshape the 6 views into individual images
    for i, img_tensor in enumerate(images):
        img_array = img_tensor.permute(1, 2, 0).numpy()  # Convert from (C, H, W) to (H, W, C)
        img_pil = Image.fromarray((img_array * 255).astype(np.uint8))  # Convert to PIL image and scale to [0, 255]
        img_name = f"{name}_view_{i+1}.png"
        img_pil.save(os.path.join(image_path, img_name))
        print(f"Saved image {img_name} to {os.path.join(image_path, img_name)}")

        outputs.append({'name': f"{name}_view_{i+1}", 'images': img_tensor})
...
# 명령어
#python run.py configs/instant-mesh-large.yaml examples --export_texmap